...
class Function:
    ...
    def f2(self, x):
        return 2 * self.T_0 * self.alpha(x) / self.R
...
class Lab4(Function):
    ...
    def A(self, T, n):
        return self.t * (self.lamda(T[n]) + self.lamda(T[n - 1])) / self.SHAG / 2

    def D(self, T, n):
        return self.t * (self.lamda(T[n]) + self.lamda(T[n + 1])) / self.SHAG / 2

    def B(self, x, T, n):
        return self.A(T, n) + self.D(T, n) + self.c(T[n]) * self.SHAG + self.p(x) * self.t * self.SHAG

    def G(self, x, T, n, ti):
        g1 = self.c(T[n]) * self.SHAG * T[n]
        g2 = self.SHAG * self.t * self.k(T[n]) * self.F0(ti) * e**(-self.k(T[n]) * x)
        g3 = self.f2(x) * self.SHAG * self.t
        return g1 + g2 + g3
    ...
    def left_bc(self, T_prev, time):
        ...
        k3 = self.t * (self.lamda(T_prev[0]) + self.lamda(T_prev[1])) / h / 2
        k4 = self.t * h * (self.p(0) + self.p(h)) / 16
        k5 = self.t * h * self.p(0) / 4
        k6 = self.alpha0 * self.t
        K0 = k1 + k2 + k3 + k4 + k5 + k6

        m1 = h * (self.c(T_prev[0]) + self.c(T_prev[1])) / 16
        m2 = self.t * (self.lamda(T_prev[0]) + self.lamda(T_prev[1])) / h / 2
        m3 = self.t * h * (self.p(0) + self.p(h)) / 16
        M0 = m1 - m2 + m3

        p1 = h * (self.c(T_prev[0]) + self.c(T_prev[1])) * (T_prev[0] + T_prev[1]) / 8 / 2
        p2 = h * self.c(T_prev[0]) * T_prev[0] / 4
        p3 = self.alpha0 * self.T_0 * self.t
        p4 = 2 * self.T_0 / 2 / self.R * (3 * self.alpha0 + self.alpha(h)) * h * self.t / 4
        p5 = h * self.t * self.F0(time) * self.k(T_prev[0]) * e**(-self.k(T_prev[0]) * 0) / 4

        p6 = h * self.t * self.F0(time) * (self.k(T_prev[0]) + self.k(T_prev[1])) / 2 * e**(-(self.k(T_prev[0]) + self.k(T_prev[1])) / 2 * (0 + h / 2)) / 4
    ...
    def right_bc(self, T_prev, time):
        ...
        k3 = self.t * (self.lamda(T_prev[-1]) + self.lamda(T_prev[-2])) / h / 2
        k4 = self.t * h * self.p(self.l) / 4
        k5 = self.t * h * (self.p(self.l) + self.p(self.l - h)) / 16
        k6 = self.alphaN * self.t
        KN = k1 + k2 + k3 + k4 + k5 + k6

        m1 = h * (self.c(T_prev[-1]) + self.c(T_prev[-2])) / 16
        m2 = self.t * (self.lamda(T_prev[-1]) + self.lamda(T_prev[-2])) / h / 2
        m3 = self.t * h * (self.p(self.l) + self.p(self.l - h)) / 16
        MN = m1 - m2 + m3

        p1 = h * (self.c(T_prev[-1]) + self.c(T_prev[-2])) / 2 * (T_prev[-1] + T_prev[-2]) / 8
        p2 = self.t * self.alphaN * self.T_0
        p3 = self.t * h * (3 * self.f2(self.l) + self.f2(self.l - h)) / 8
        p4 = h * self.c(T_prev[-1]) * T_prev[-1] / 4
        p5 = h * self.t * self.F0(time) * self.k(T_prev[-1]) * e**(-self.k(T_prev[-1]) * self.l) / 4
        p6 = h * self.t * self.F0(time) * (self.k(T_prev[-1]) + self.k(T_prev[-2])) / 2 * e**(-(self.k(T_prev[-1]) + self.k(T_prev[-2])) / 2 * (self.l - h / 2)) / 4
...
class Res(Lab4):
    ...
    def draw(self):
        ...
        te = [i for i in np.arange(0, ti, self.t)]
...
# группа 2д графиков
    ...
    def chast(self): # надо поменять в F0: return 0 поставить
        ...
        while (i < ti):
            ...
            i += self.t
        i = 0
        xfix = [temp[int(i / self.SHAG)] for temp in res]
        plt.plot(te, xfix[1:])
        plt.xlabel("Время, c")
        plt.ylabel("Температура, K")
        plt.show()
